...
// #elif defined(HELTEC_LORA_V3)
  ...
  void VextOn(void) {
      ...
      // digitalWrite(VEXT_CTRL, HIGH);  // VExt_Ctrl active HIGH
  ...
  void VextOff(void) {
      // pinMode(VEXT_CTRL, OUTPUT);
      // digitalWrite(VEXT_CTRL, LOW);  // VExt_Ctrl active HIGH
      display.displayOff();
  }
...
void heltec_loop() {
  ...
  if (button.pressedFor(1000)) {
    ...
    // // Visually confirm it's off so user releases button, MAYBE NOT NEEDED
    // #ifdef HELTEC_WIRELESS_TRACKER
    //   st7735.enableSleep(true);
    // #elif HELTEC_LORA_V3
    //   display.displayOff();
  ...
  //   Serial.printf("button 1 Click\n");
    ...
    Serial.printf("button 2 Click\n");
  } else if (button.isTripleClick()) {
    Serial.printf("button 3 Click\n");
  } else if (button.isQuadrupleClick()) {
    Serial.printf("button 4 Click\n");
  }
